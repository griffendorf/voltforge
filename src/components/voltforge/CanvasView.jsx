...
export default function CanvasView({
  ...
  issuesByComp, aiHL,
  onCanvasTap, onCompPress, onTermPress, setWColor, setSelected, bump
}) {
  return (
    <div style={{ width:'100%', height:'100%', position:'relative' }}>
      {placing && (
        <div style={{ position:'absolute', top:8, left:'50%', transform:'translateX(-50%)',
                      ...
                      pointerEvents:'none', background:`${T.blue}18`,
                      border:`1px solid ${T.blue}55`, color:T.blue, whiteSpace:'nowrap',
                      animation:'pulse 1.2s ease-in-out infinite' }}>
          Tap canvas to place {DEFS[placing]?.label} · tap ＋ to cancel
        </div>
      )}
      {isDrawing && !placing && (
        <div style={{ position:'absolute', top:8, left:'50%', transform:'translateX(-50%)',
                      zIndex:50, padding:'5px 16px', borderRadius:20, fontSize:10,
                      pointerEvents:'none', background:`${T.cyan}14`,
                      border:`1px solid ${T.cyan}44`, color:T.cyan, whiteSpace:'nowrap' }}>
          Drag to a terminal to connect
        </div>
      )}
      ...
      <div ref={cvRef} data-cv="true"
        onMouseDown={onCanvasTap} onTouchStart={onCanvasTap}
            ...
            return (
              <g key={w.id}>
                ...
                <path d={d} fill="none" stroke={col} strokeWidth={2.5}
                      strokeLinecap="round"
                      style={{ strokeDasharray: act ? '10 6' : undefined,
          ...
          {rubberPath && (
            <>
              <path d={rubberPath} fill="none"
                    stroke={snapTarget?.valid ? T.green : snapTarget ? T.red : wColor}
          ...
          return (
            ...
            >
              <div style={{ position:'absolute', inset:0, borderRadius:13,
                ...
                <span style={{ fontSize:8, color:T.sub, letterSpacing:'.04em', position:'relative',
                ...
                {((comp.type==='fuse'&&comp._blown)||(comp.type==='breaker'&&comp._tripped)) && (
                  <button
                    ...
                    onClick={e => { e.stopPropagation();
                      ...
                      if(comp.type==='breaker') comp._tripped=false;
                      bump(); }}
                    style={{ position:'absolute', bottom:3, right:3, width:18, height:18,
              ...
              {isSel && (
                <>
                  <button
                    ...
                    style={{ position:'absolute', top:-11, right:-11, width:26, height:26,
                             borderRadius:'50%', border:'none', background:T.red, color:'#fff',
                             fontSize:14, zIndex:30, display:'flex', alignItems:'center',
                             justifyContent:'center', boxShadow:`0 0 8px ${T.red}88`,
                             touchAction:'none' }}>✕</button>
                  <button
                    onMouseDown={e => { e.stopPropagation(); e.preventDefault(); }}
                    onTouchStart={e => { e.stopPropagation(); e.preventDefault(); }}
                    ...
                    style={{ position:'absolute', top:-11, left:-11, width:26, height:26,
                             borderRadius:'50%', border:'none', background:T.purple, color:'#fff',
                             fontSize:14, zIndex:30, display:'flex', alignItems:'center',
                             justifyContent:'center', boxShadow:`0 0 8px ${T.purple}88`,
                             touchAction:'none' }}>↻</button>
                </>
              )}
              ...
              {comp.termIds.map(tid => {
                ...
                if (!t) return null;
                const isFrom = false; // drawing ref handled externally
                ...
                return (
                  <div key={tid}
                    ...
                    style={{ position:'absolute', left:t.lp.x-sz/2, top:t.lp.y-sz/2,
